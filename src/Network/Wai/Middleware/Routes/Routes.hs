...
-- Yesod Routes
import Yesod.Routes.Class (Route, RenderRoute(..), ParseRoute(..))
import Yesod.Routes.Parse (parseRoutes, parseRoutesNoCheck, parseRoutesFile, parseRoutesFileNoCheck, parseType)
import Yesod.Routes.TH (mkRenderRouteInstance, mkParseRouteInstance, mkDispatchClause, ResourceTree(..), MkDispatchSettings(..), defaultGetHandler)
...
mkRoute typName routes = do
  ...
  -- TODO: I don't know what route 'attrs' are.
