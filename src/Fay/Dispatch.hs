...
toPieceMap' depth rhs = PieceMap
    ...
    , pmStatic = textMapMap (toPieceMap' depth') statics
    }
  where
    depth' = depth - 1
...
-- appropriate here, since it would only keep one route per Text.
    ...
    getStatic (Static t, rh) = Just $ textMapSingleton t [rh]
    getStatic _ = Nothing
    statics = textMapUnionsWith (++) $ mapMaybe getStatic pairs
...
-- is greater than *max(n)*, then use the "rest" group.
    ...
    pm0 = fromMaybe rest $ vec !? length ts0
...
append (PieceMap a x) (PieceMap b y) =
    PieceMap (append a b) (textMapUnionWith append x y)
...
(!?) :: [a] -> Int -> Maybe a
a !? b = if b < 0 then Nothing else go a b
  where go []    _ = Nothing
        go (h:_) 0 = Just h
        go (_:t) n = go t (n-1)
